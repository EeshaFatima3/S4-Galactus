...
void load_floats(const char *filename, float *array, size_t N) {
    ...
    fclose(fp);
    printf("Loaded %zu floats from %s\n", N, filename);
}
...
int main() {
    ...
    // Printing first 5 outputs for debug
    printf("\nFirst 5 computed vs expected outputs:\n");
    for (size_t i = 0; i < 5 && i < L * Dout; i++) {
        printf("Y[%zu] = %.6f, Y_expected[%zu] = %.6f\n", i, Y[i], i, Y_expected[i]);
    }
